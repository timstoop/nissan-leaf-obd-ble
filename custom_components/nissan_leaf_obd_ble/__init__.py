...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry):
    ...
    async def _run_advert_wakeup() -> None:
        ...
        agent_log("__init__:_run_advert_wakeup", "task_start", {"address": address}, "H1")
        try:
            await coordinator.async_request_refresh()
        except Exception as err:  # noqa: BLE001
            agent_log(
                "__init__:_run_advert_wakeup", "task_exception",
                {"err_type": type(err).__name__, "err": str(err)[:200]}, "H3",
            )
            raise
        dt = _time_monotonic.monotonic() - t0
        agent_log("__init__:_run_advert_wakeup", "after_async_request_refresh", {"seconds": round(dt, 4)}, "H1")
    ...
    def _async_specific_device_found(
        ...
        agent_log("__init__:_async_specific_device_found", "callback_fired", {"change": str(change)}, "H3")
